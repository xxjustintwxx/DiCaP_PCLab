...
def get_datasets(args):
    ...
    ub_train_dataset = data_handler(ub_train_imgs, ub_train_labels, dataset_dir, transform=train_transform)

    val_dataset = data_handler(source_data['val']['images'], source_data['val']['labels'], dataset_dir, transform=test_transform)
...
class CutoutPIL(object):
    ...
    def __call__(self, x):
        ...
        y_c = torch.randint(0, h, (1,)).item()
        x_c = torch.randint(0, w, (1,)).item()
        y1 = max(0, y_c - h_cutout // 2)
        y2 = min(h, y_c + h_cutout // 2)
        x1 = max(0, x_c - w_cutout // 2)
        x2 = min(w, x_c + w_cutout // 2)
...
class TransformWithPatches_Train(object):
    ...
    def __call__(self, img):
        ...
        x_order = torch.randperm(self.grid_perside).tolist()
        y_order = torch.randperm(self.grid_perside).tolist()
